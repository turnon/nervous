...
func (tq *TagQuery) QueryEvents() *EventQuery {
...
func (tq *TagQuery) WithEvents(opts ...func(*EventQuery)) *TagQuery {
	query := &EventQuery{config: tq.config}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withEvents; query != nil {
		...
		query.Where(predicate.Event(func(s *sql.Selector) {
			s.Where(sql.InValues(tag.EventsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.tag_events
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tag_events" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tag_events" returned %v for node %v`, *fk, n.ID)
			}
