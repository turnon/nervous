...
func (tu *TagUpdate) AddEvents(e ...*Event) *TagUpdate {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
...
func (tu *TagUpdate) ClearEvents() *TagUpdate {
...
func (tu *TagUpdate) RemoveEvents(e ...*Event) *TagUpdate {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
...
func (tu *TagUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if tu.mutation.EventsCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: []string{tag.EventsColumn},
			Bidi:    false,
...
// AddEvents adds the "events" edges to the Event entity.
func (tuo *TagUpdateOne) AddEvents(e ...*Event) *TagUpdateOne {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
...
// ClearEvents clears all "events" edges to the Event entity.
func (tuo *TagUpdateOne) ClearEvents() *TagUpdateOne {
...
// RemoveEvents removes "events" edges to Event entities.
func (tuo *TagUpdateOne) RemoveEvents(e ...*Event) *TagUpdateOne {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
